...
function borrar_imagen(image_preview,input_borrar){
   $("#"+image_preview).hide();
   $("#"+input_borrar).val(1);
}
...
function borrarPagoSession(id){
    if(confirm("�Esta seguro que desea eliminar el pago?")) { 
    $('#pagos_quitar').load('/Venta/pago/quitar.php?id='+id);
    }
}

function borrarPagoBd(id){
    if(confirm("�Esta seguro que desea eliminar el pago?")) { 
    $('#pagos_quitar').load('/Venta/pago/eliminar_bd.php?id='+id);
    }
}

function limpiarBsqMotoService(){
    //alert("asdsad");
    
    
    $('#cli_dni_frm').val('');
    $('#cli_nombre_frm').val('');
    $('#cli_apellido_frm').val('');
...
function asignarMotoPlan(id){
    window.open('/Venta/plan_ahorro/listado.php?id='+id, 'Popup', 
    'height=550,width=1200,scrollTo,resizable=1,scrollbars=1,location=0');
}

function asignarMotoBD(id_venta_lis,id_moto_lis){
    if(confirm("�Esta seguro que desea asignar esta moto?")) { 
    $('#ajax_call').load('/Venta/plan_ahorro/asignarPlanMotoBD.php?venta='+id_venta_lis+'&moto='+id_moto_lis);
    }
}


function autoCargaPrecioMoto(){//modelo
    $('#script_modelo_precio').load('/moto/autocarga_precio.php?modelo='+$('#modelo').val());
    //alert($('#modelo').val());
}