...
if(isset($post_nombre) && trim($post_nombre)!=''){
    $criterio_busqueda.=" AND vc_titulo LIKE '%".addslashes($post_nombre)."%' ";
	$listado->setVariable("nombre",  htmlspecialchars($post_nombre));
}

if(isset($post_curso) && trim($post_curso)!=''){
    $criterio_busqueda.=" AND vc.curso_id=$post_curso";
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
$qry="SELECT * FROM video_curso vc LEFT JOIN curso c on (vc.curso_id=c.curso_id) "
...
if(count($resultados)){
    foreach ($resultados as $rs){
        ...
        $listado->setVariable("titulo",$rs["vc_titulo"]);
        

        if($_SESSION["accion_activa"]["acc_eliminar"]){
        $listado->setVariable("id_eliminar",$rs["vc_id"]);
        $listado->setVariable("acc",$_GET["acc"]);
        }
        if($_SESSION["accion_activa"]["acc_editar"]){
        $listado->setVariable("id_editar",$rs["vc_id"]);
