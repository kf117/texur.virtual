...
if(isset($post_nombre) && trim($post_nombre)!=''){
    $criterio_busqueda.=" AND ac_nombre LIKE '%".addslashes($post_nombre)."%' ";
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
$qry="SELECT * FROM archivo_curso ac LEFT JOIN curso c on (ac.curso_id=c.curso_id) "
...
if(count($resultados)){
    foreach ($resultados as $rs){
        ...
        $listado->setVariable("titulo",$rs["ac_nombre"]);
        
        if(file_exists(dirname(dirname(dirname(dirname(__FILE__))))."/archivos/curso/".$rs["ac_archivo"]) && $rs["ac_archivo"]!="")
        	$listado->setVariable("archivo_lis",$rs["ac_archivo"]);
        else
        	$listado->setVariable("no_aviable","");
        

        if($_SESSION["accion_activa"]["acc_eliminar"]){
        $listado->setVariable("id_eliminar",$rs["ac_id"]);
        $listado->setVariable("acc",$_GET["acc"]);
        }
        if($_SESSION["accion_activa"]["acc_editar"]){
        $listado->setVariable("id_editar",$rs["ac_id"]);
