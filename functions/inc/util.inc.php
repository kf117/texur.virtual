<?php
   ...
   define('HEAD_CACHE_PATH', $path . "/functions/tpl/cache");
	...
	function obtenerExtension($file){
        ...
        function mime_type_permitidos($type,$allowed_types){
...
function armarMenu($db,$template,$perfil_id,$acc,$conf,$idioma_id){
    $db->rawData("SELECT * FROM idioma");
    ...
    foreach ($acciones_grales as $ag){
        ...
        $template->setVariable("url_accion",$ag["acc_url"]);
        ...
        if(count($acciones_hijos)){
            ...
            foreach($acciones_hijos as $ah){
               if($acc==$ah["acc_id"]){
                $template->setVariable("accion_activa","active");
                $template->setVariable("accionh_activa","active");
                $accion_activa=$ah;
               }
               if($ah["acc_ver"]==0){
               ...
               $template->setVariable("url_accionh",$ah["acc_url"]);
    ...
    if($acc>0 && trim($accion_activa["acc_archivo"])!="" ){
        ...
        include(dirname(dirname(dirname(__FILE__)))."/admin_cms/".$accion_activa["acc_archivo"]);
        
    }else{ 
        inicializar($bienvenida,  dirname(dirname(dirname(__FILE__)))."/admin_cms/interfaces/mensaje_bienvenida.html");
...
function borrar_sesiones(){
    ...
    $aux_idioma=$_SESSION["idioma_gestor"];
     session_unset();
    $_SESSION["usuario_gestor"]=$aux;
    $_SESSION["idioma_gestor"]=$aux_idioma;
}
